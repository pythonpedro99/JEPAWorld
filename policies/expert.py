...
from helpers import find_path, smooth_path
...
class ExpertPolicy:
    ...
    def __init__(self, env: gym.Env, graph: nx.Graph, nodes, nodes_positions, obstacles, mission: list[str]):
        # Basics 
        self.env = env
        self.graph = graph
    ...
    def go_to(self, goal: str) -> None:
        ...
        # 2) Plan & smooth
        path = find_path(self.graph,
                        self.nodes, 
                        self.env.unwrapped.agent.pos,
                        goal)
        self.path += path
        smoothed_path = smooth_path(path, self.chaikins_iterations)
        self.smoothed_path += smoothed_path
        if not smoothed_path:
            return

        # 3) Precompute goal point
        goal_pt = Point(smoothed_path[-1][0], smoothed_path[-1][1])
